...
const DEFAULTMAXFILESIZE = 1024 * 1024 * 1024
const DEFAULTMAXJSONSIZE = 1024 * 1024

// Tools is te type used to instantiate this module. Any variable of this type will have access
// to all the methods with the received *Tools
type Tools struct {
	MaxFileSize        int
	MaxJSONSize        int
...
// It return a slice contraining the newly named files, the original file names, the size of the file
// and potentially an error. If the optional last parameter is set to true, then we will not rename the files
	...
	for _, fHeaders := range r.MultipartForm.File {
		for _, hdr := range fHeaders {
			uploadedFiles, err = func(uploadedFiles []*UploadedFile) ([]*UploadedFile, error) {
				...
				// Copy the first 512 bytes to buffer to check for file type
...
// JSONResponse is a type use for sending JSON around
...
func (t *Tools) ReadJSON(w http.ResponseWriter, r *http.Request, data interface{}) error {
	maxBytes := DEFAULTMAXJSONSIZE // one meg
	if t.MaxJSONSize != 0 {
